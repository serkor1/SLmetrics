...
A generic S3 function to compute the *symmetric mean absolutte
percentage error* score for a regression model. This function dispatches
to S3 methods in `smape()` and performs no input validation. If you
...
Because `smape()` operates on raw pointers, pointer‑level faults (e.g.
...
<tr class="odd">
<td><code id="...">...</code></td>
<td><p>Arguments passed on to <code>smape.numeric</code>,
<code>weighted.smape.numeric</code></p>
<dl>
...
`rrmse()`, `rrse()`, `rsq()`, `shannon.entropy()`, `specificity()`,
`zerooneloss()`
