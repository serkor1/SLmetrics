...
|                  |                 |
|------------------|----------------:|
| deviance.poisson | R Documentation |
...
`deviance.poisson()` and performs no input validation. If you supply NA
...
Because `deviance.poisson()` operates on raw pointers, pointer‑level
...
To guard against this, wrap `deviance.poisson()` in a “safe” validator
...
<tr class="odd">
<td><code id="...">...</code></td>
<td><p>Arguments passed on to <code>deviance.poisson.numeric</code>,
<code>weighted.deviance.poisson.numeric</code></p>
<dl>
...
Other Regression: `ccc()`, `deviance.gamma()`, `deviance.tweedie()`,
`huberloss()`, `mae()`, `mape()`, `mpe()`, `mse()`, `pinball()`,
`rae()`, `rmse()`, `rmsle()`, `rrmse()`, `rrse()`, `rsq()`, `smape()`
