...
|                  |                 |
|------------------|----------------:|
| roc.curve.factor | R Documentation |
...
roc.curve(actual, response, thresholds = NULL, presorted = FALSE, ...)
```
...
`relative.entropy()`, `shannon.entropy()`, `specificity()`,
`zerooneloss()`
...
`recall()`, `relative.entropy()`, `rmse()`, `rmsle()`, `rrmse()`,
