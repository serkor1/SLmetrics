...
Below is four arguments of why {SLmetrics} should be considered in your ML pipeline,
...
In both cases the execution time is diverging in favor of {SLmetrics}; we promised speed and efficiency - and that is what you get.
...
Secondly, {SLmetrics} is *simple* and *flexible* to use; it is based on `S3` and provides the most essential class-wise and aggregated metrics.
...
{SLmetrics} is build as the `R`-version of [{scikit-learn}](https://github.com/scikit-learn/scikit-learn) but with a larger focus on versatility, speed and the simplicity of `R`. All the functions implemented in {SLmetrics} are tested using [{scikit-learn}](https://github.com/scikit-learn/scikit-learn) and [{pytorch}](https://github.com/pytorch/pytorch) as reference values. 
