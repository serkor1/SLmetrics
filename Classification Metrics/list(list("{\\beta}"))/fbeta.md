...
A generic S3 function to compute the *`f_{\beta}`* score for a
classification model. This function dispatches to S3 methods in
`fbeta()` and performs no input validation. If you supply NA values or
...
Because `fbeta()` operates on raw pointers, pointer‑level faults (e.g.
...
To guard against this, wrap `fbeta()` in a “safe” validator that checks
...
The `fbeta.factor()` method calls `cmatrix()` internally, so explicitly
invoking `fbeta.cmatrix()` yourself avoids duplicate computation,
...
<tr class="odd">
<td><code id="...">...</code></td>
<td><p>Arguments passed on to <code>fbeta.factor</code>,
<code>weighted.fbeta.factor</code>, <code>fbeta.cmatrix</code></p>
<dl>
<dt><code>beta</code></dt>
<dd>
<p>A &lt;double&gt; vector of length <code class="reqn">1</code>
(default: <code class="reqn">1</code>).</p>
</dd>
...
<code>sum(c(1, 2, NA), na.rm = TRUE) / length(c(1, 2, NA))</code>.</p>
</dd>
<dt><code>w</code></dt>
<dd>
...
`cross.entropy()`, `dor()`, `fdr()`, `fer()`, `fmi()`, `fpr()`,
...
`deviance.poisson()`, `deviance.tweedie()`, `dor()`, `fdr()`, `fer()`,
