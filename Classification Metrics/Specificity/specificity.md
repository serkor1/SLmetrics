...
`specificity()` and performs no input validation. If you supply NA
...
Because `specificity()` operates on raw pointers, pointer‑level faults
...
To guard against this, wrap `specificity()` in a “safe” validator that
...
The `specificity.factor()` method calls `cmatrix()` internally, so
explicitly invoking `specificity.cmatrix()` yourself avoids duplicate
...
<tr class="odd">
<td><code id="...">...</code></td>
<td><p>Arguments passed on to <code>specificity.factor</code>,
<code>weighted.specificity.factor</code>,
<code>specificity.cmatrix</code></p>
<dl>
