# objective: Test that the gamma deviance metric
# implemented in {SLmetrics} is aligned with
# target functions.
testthat::test_that(
  desc = "Test `gamma.deviance()`-function", code = {
    testthat::skip_on_cran()
    
    # 0) construct gamma.deviance-wrapper
    wrapped_gamma_deviance <- function(
      actual,
      predicted,
      w = NULL) {
      if (is.null(w)) {
