...
* **Mean Arctangent Absolute Error:** See [here](https://support.numxl.com/hc/en-us/articles/115001223463-MAAPE-Mean-Arctangent-Absolute-Percentage-Error) for a general description of the implementation. The metric can be calculated as follows:
...
The entropy functions have had the `base`-argument removed, and a new argument have been introduced: `normalize`. The `normalize`-parameter averages the calculated entropy across the desired dimensions.
...
* **Poisson Logloss:** The `logloss()` for count data `logloss.integer()` were taking a `matrix` of probabilities. This has been change to a `vector` of probabilities.